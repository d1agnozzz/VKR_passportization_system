...
DatasetInfo = collections.namedtuple(
    "DatasetInfo", "num_classes, label_divisor, thing_list, colormap, class_names"
)
...
def color_panoptic_map(
    panoptic_prediction, dataset_info: DatasetInfo, perturb_noise, slam_masking: bool = True,
):
    ...
    colored_panoptic_map = np.full((height, width, 3), fill_value=255, dtype=np.uint8)
    ...
    for semantic_id in unique_semantic_ids:
        ...
        if slam_masking:
            if semantic_id < 0 or semantic_id >= len(dataset_info.class_names):
                print(f'Unknown semantic ID! id: {semantic_id}')
                colored_panoptic_map[semantic_mask] = [0,0,0]
                continue
                
            if dataset_info.class_names[semantic_id] in DISCARD_CLASSES:
                colored_panoptic_map[semantic_mask] = [0, 0, 0]
                used_colors[semantic_id].add((0,0,0))
                continue
            else:
                continue
        ...
        if semantic_id in dataset_info.thing_list:
            ...
            for instance_id in unique_instance_ids:
                instance_mask = np.logical_and(
                    semantic_mask, instance_map == instance_id
                )
...
def vis_segmentation(image, panoptic_prediction, dataset_info, perturb_noise=60, masking_slam:bool = True):
    ...
    panoptic_map, used_colors = color_panoptic_map(
        panoptic_prediction, dataset_info, perturb_noise, slam_masking=masking_slam
    )
    if masking_slam:
        masked_image = np.bitwise_and(image, panoptic_map)
        plt.imshow(masked_image)
        plt.imsave("slam_masked.png", masked_image)
        plt.axis('off')
        ax.set_title('masked slam', fontsize=20)
    else:
        plt.imshow(panoptic_map)
        plt.imsave("panoptic_result.png", panoptic_map)
        plt.axis("off")
